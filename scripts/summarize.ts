/**
 * 자막을 LLM map-reduce로 요약해 data/meetings/*.json 을 생성한다.
 *
 * 1) map    — 15분 단위 청크별로 발언 단위 분리 + 화자 추론 (light 모델, speakers.json 명부 주입)
 * 2) reduce — 청크 결과를 통합해 구조화 요약(안건/발언 스레드/지시/AI·데이터 정책) 생성 (main 모델)
 * 3) link   — 기존 회의의 미결 지시와 새 회의 내용을 대조해 후속 보고를 "추정 연결" (light 모델)
 *
 * LLM 공급자·모델 선택은 scripts/llm.ts 참고 (Claude / OpenAI GPT / OpenRouter).
 * 선택 환경변수: MAX_MEETINGS(1회 실행당 처리 수, 기본 3)
 */
import fs from "fs";
import path from "path";
import { pathToFileURL } from "url";
import { ask, assertLlmEnv, describeLlm } from "./llm";
import {
  DATA_DIR,
  extractJson,
  log,
...
(매칭 없으면 {"matches":[]})`;
  const text = await ask(prompt, 2000, "light");
